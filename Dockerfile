...
# Add PostgreSQL JDBC driver for Tomcat
...
# Startup script: wait for DB, init schema, then start Tomcat
...
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f /init.sql || echo "⚠️  Database already initialized, skipping."
